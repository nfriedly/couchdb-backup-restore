  ...
  getDbs(options, connection, function (err, dbs) {
    ...
    async.forEach(dbs, function (dbname, next) {
      ...
      db.list(params, function (err, body) {
        ...
        if (body.rows.some(function(row) { return row.doc._attachments; })) {
          console.error("WARNING: couchdb-backup-restore does not currently support attachments. At least one document in %s has attachments that will not be included in the backup.", dbname)
        }
